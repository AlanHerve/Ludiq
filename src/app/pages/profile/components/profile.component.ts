...
export class ProfileComponent implements Image {
    ...
    this.activatedRoute.params.subscribe((params) => {
      this.userService.findUserById(parseInt(params['id'])).subscribe({
        next: (user) => {
          ...
          this.friendService
            .isFriendWith(
              parseInt(JSON.parse(localStorage.getItem('currentUser')!).id),
              this.profileDTO.userDTO.id
            )
            .subscribe({
              next: (response) => {
              ...
              error: (error) => {
                console.log(
                  'Error while finding if the user is friend with another',
                  error
                );
              },
            });
          ...
          this.hobbyService.currentDeleteState.subscribe((data) => {
            ...
            this.hobbyFlashcardsDTO.splice(
              this.findHobbyDTOWithData(data),
              1
            );
            this;
          });
  ...
  getProfileInformation(): void {
    this.profileService
      .getProfileInformation(this.profileDTO.userDTO.id)
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          console.log(
            'error while accessing to profile informations : ',
            error
          );
        },
      });
  ...
  removeFriend(): void {
    ...
    this.friendService
      .removeFriend(id_user, this.profileDTO.userDTO.id)
      .subscribe({
        next: (response) => {
  ...
  changeFavoriteHobby(): void {
    this.isChoosingFavoriteHobby = true;
    this.hobbyService
      .getHobbiesOfUser(this.profileDTO.userDTO.id)
      .subscribe({
        next: (response) => {
          this.favoriteHobbyList = response;
        },
        error: (error) => {
          console.error('Could not get all hobbies', error);
        },
  ...
  selectFavoriteHobby(hobby: HobbyDTO | undefined): void {
    if (hobby) {
      this.selectedHobby = hobby;
      this.profileDTO.favoriteHobby = hobby;
      this.hobbyService
        .updateFavoriteHobby(this.profileDTO.userDTO.id, hobby.id)
        .subscribe({
          next: (response) => {
